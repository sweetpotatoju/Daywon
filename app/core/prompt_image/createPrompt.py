...
async def create_prompt(finance_category):
    ...
    user_prompt = f"다음 동작을 수행하세요. 1.{get_finance_category(finance_category)}를 주제로 정하세요. 2.정해진 주제의 금융 상품 중 하나에 대한 개념을 설명해주세요."
...
async def create_example_prompt(conceptual_prompt):
    ...
    system_prompt = """
    다음 조건들을 모두 만족하는 문장을 만들어주세요.
    각 문장의 글자 수가 80자 이내로 작성해주세요.
    총 6문장을 작성해주세요.
    영어가 아닌 한글만 사용해주세요.
     10대에게 이야기하듯이 작성해주세요.
    온점은 오로지 문장이 끝났을 때만 사용해주세요.
    """
