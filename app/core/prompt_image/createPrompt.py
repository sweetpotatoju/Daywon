...
def get_finance_category(level=None):
    ...
    if level is None:
        ...
        finance_categories = {'자산 배분', '위험 관리', '수익률 분석', '재무제표 분석', '손익계산서', '대차대조표', '자본 구조', '기업 재무', '기업 가치 평가', '환율', '외환 거래', '해외 투자'}
    elif (level == 5):
        finance_categories = {'M&A (인수합병)', '벤처 캐피탈', '사모펀드 재무 비율 분석', '현금 흐름 분석', '자본 비용 상속세', '증여세', '고급 세금 계획', '블록체인', '암호화폐', '디지털 뱅킹', '금융 기술 혁신'}
