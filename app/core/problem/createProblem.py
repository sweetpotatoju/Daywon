...
async def create_problem(script, example_script):
    ...
    system_prompt = f"초등학교 수준에서 이해할 수 있는 {script}와{example_script}에서 언급한 내용을 이용하여 객관식 금융 문제를 만들고, 이에 대한 해설도 제공해주세요."
    user_prompt = (f"문제: {script}와 {example_script}에서 언급한 내용을 이용하여 객관식 문제 하나를 만들어주세요."
                  f"문제 형식은 객관식이며, 선택지는 네 개입니다.문제의 답은 확실한 정답은 한 개이어야 합니다. 선택지는 1,2,3,4로 표현해주세요. "
