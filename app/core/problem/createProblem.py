...
async def create_problem(result):

    api_key = get_api_key()
    model='ft:gpt-3.5-turbo-0125:personal:daywon123:9HulgDod'
    system_prompt = f"초등학교 수준에서 이해할 수 있는 {result}와 관련된 객관식 금융 문제를 만들고, 이에 대한 해설도 제공해주세요."
    user_prompt = (f"문제: {result}에 관련된 객관식 문제 하나를 만들어주세요."
